...
{\label{bio_results}  {\bf Venn diagram of fusion read integration points recovered from the TCGA-CESC samples with matched RNA-Seq and WGS sequencing.}  We show a Venn diagram of the fusion read mRNA integration sites reported by our method, VERSE, and the Larsson et al. 2014 study on TCGA-CESC samples with both RNA-Seq and WGS sequencing matched pair data.  Only mRNA fusion reads within 100kb of a WGS integration reported by our method are considered for creating the Venn diagram.  A total of 315 fusion mRNA integration points are reported, of which, 237 are found by by our method.}
\end{figure}
...
{\label{integration_counts}  {\bf Annotations of integrated regions.}  We report the number of annotations within a 10kb interval of each integration.  We also report the expected counts from 1000 simulated experiments.  }
\end{figure}
