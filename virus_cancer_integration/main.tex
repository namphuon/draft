...
al. study, we investigated if this could extend to all (68) TCGA-CESC
samples with matched RNA data, and if the combined analysis could
provide new insights into the pathogenicity of HPV in Cervical cancer.
...
of a genomic integration (Fig.~\ref{distance_breakpoint}); 92\% of
these were within 50kbp, and 70\% within 10kbp. The larger distances
are consistent with splicing events. On the other side, 119 (52\%) of
the genomic integrations (119 genomic integrations) had at least one
fusion mRNA sequence. Previous studies have shown extensive
...
looked at the annotations near integratiion points. 67\% of the
...
HPV integration. However, we observed only two recurrent integrations
(integrations from two or more samples within 50kb of each other) at
two loci: two samples with integrations in the region
chr13:73955151-74005092 and four samples with integrations in the
region chr8:128747810-128889296.  The first region contains no nearby
genes, and the latter region contains the oncogene MYC and PVT1. Both
have been implicated in recurrent integrations~\cite{Hu2015}. Hu et
al. (2015) also report an enrichment of genomic instability-related
elements near integration points, including short tandom repeats
(STR), short interspersed nuclear element (SINE-Alu), long terminal
repeat/endogenous retroviruses (LTR/ERV1), fragile X breakpoint
cluster repeats, and Satellite. However, these elements are ubiquitous
in the genome, and we did not find any enrichment in our dataset
(Methods)\footnote{NOTE: I am currently re-running this right now, but
  I suspect this will hold, but will update this statement once it's
  done.  Elements are short tandom repeats (STR), short interspersed
  nuclear element (SINE-Alu), long terminal repeat/endogenous
  retroviruses (LTR/ERV1), fragile X breakpoint cluster repeats, and
  Satellite}.  Finally, while we did find a slight enrichment of
oncogenes compared to randomly selected intervals, this finding was
not significant.  Thus, the vast majority of integrations were not
recurrent, nor were they in oncogenes. In totality, we cannot assert
that the location of HPV genomic integration plays any role in CESC
pathogenicity, and we decided to look at the RNA evidence near
integration.

For each integration, we examined the total transcription activity of
the region within 10kb of the sample containing the integration by
comparing region's normalized transcriptional activity as against the
transcriptional activity of the same region in other samples without
genomic integration.  We find a significant increase ($2.83\times$
average) in across all genomic segments with integrations
(Fig.~\ref{all_expression}).

Remarkably, the increased expression is highly correlated with the
presence of a fusion transcript (Fig.~\ref{expression_transcripts}).
There is a $7.41\times$ average increase if the genomic segment
contains a fusion mRNA sequence, versus a small decrease
($0.97\times$) when no fusion mRNA is seen.  Previous genome-wide
surveys of viral integration sites explored the role of integration on
gene expression~\cite{Sung2012,Hu2015}\VB{Why cite this?}; our results
suggest that integration typically results in upregulation of local
transcriptional activity, regardless if genes are present or not, and
this upregulation could potentially disrupt regulatory networks in
unknown and unexpected ways\VB{Not shown here}.

\begin{figure}[htpb]
  \centering
  \includegraphics[ width=1\linewidth]{{results/wgs.fpkm.fusion.boxplot.10000}.pdf}\\
\caption[Log-fold change of expression between genomic segments with
  and without integrations.]  {\label{all_expression} {\bf Boxplots of
    ...
    without integrations.}  We show boxplots of the Log2-fold change
  ...
  fusion transcriptions with the genomic integration.}
\end{figure}

The observation that integration often results in increased expression
leaves us with two fundamental questions.  First, how does the
increased expression contribute to pathogenesis.  Second, what causes
the increase in expression, and in particular, why is this increase
more prominent when fusion mRNA sequences are observed.

To answer the first question, we started by annotating the expressed
transcripts proximal to genomic integrations. For each genomic segment
containing an integration, we took all positions within 10kb of the
integration that were covered by at least three or more reads, and
then clustered the positions into intervals.  We merged any intervals
that were within 5 bps of another interval.  We then counted the
number ofannotations that intersected any of the intervals using
RefSeq genes~\cite{OLeary2016} or
RepeatMasker~\cite{Tarailo-Graovac2009}) annotations.  We also
computed the expected number of annotations observed from randomly
selected segments as a baseline for comparison.  We found that regions
with integrations were statistically significantly enriched for genes
and repeats (Z-test; p-value < 1e-20). Specifically, we see a two-fold
increase in the number of gene annotations of transcripts. Even more
interesting, we see more than a 12-fold increase in LINE and LTR
annotations of transcripts (Fig.~\ref{transcript_count}).  This is
particularly surprising considering that we saw no statistically
significant difference between the number of LINE and LTR annotation
at the genomic level, however, we do see a statistically significant
difference at the transcriptomic level.
...
expression of LINE and LTR elements that are not normally expressed.
L1 elements are retrotransposons that have a ``cut and paste''
mechanism which allow them to replicate through RNA intermediates and
re-insert back into genomic DNA.  L1 elements have been shown to
create double stranded breaks in the host cell genome, leading to
genomic instability~\cite{Gasior2006}.  L1 elements are typically
epigenetically repressed in somatic
cells~\cite{Kinomoto2007,Sigurdsson2012}, but they have been shown to
be active in selected neoplasms~\cite{Rodic2013,Xiao-Jie2016}.  LTRs
can act as promoters, enhancers, and transcriptional factor-binding
sites, and can potentially be transcriptionally active, depending on
the tissue~\cite{Yu2013}.  In most cases, LTRs are transcriptionally
silenced by epigenetic mechanisms. However they have been shown to
have up-regulated expression in cancer~\cite{Romanish2010}.  Our
results show a four to five-fold increase in expression of these
elements in regions proximal to integration sites
(Fig.~\ref{expression_transcripts}).  As HPV randomly integrates into
the genome, it might fall proximal to LINE and LTR elements and result
in deregulation of cellular pathways by the active transcription of
LINE and LTR elements, and in particular increased genomic instability
...
transcriptionally active in cervical cancer cells~\cite{XXX}, we can
assume that the majority of viral transcripts should be oriented in
the same direction as the viral gene.  Using this assumption, we could
determine if the human portion of a fusion transcript was upstream or
downstream of the viral gene. 82\% of fusion mRNA sequences showed the
human fragment to be downstream of the viral genes.
(Fig.~\ref{mrna_directions}). Of the remaining 18\% sequences where
the human portion was upstream of the viral gene, 87\% were within
10kb of an annotated gene.  These observations support our hypothesis
that transcription of fusion mRNA sequences are largely being driven
by the upstream regulatory elements within the viral genome.  In the
rare cases in which the viral gene is downstream of the human portion
of the fusion transcript, we suspect that regulatory elements in the
human genome are driving this expression.
...
To better characterize the integration might drive downstream
...
us to determine upstream and downstream of the integration site.  We
define complex integrations as regions containing multiple multiple
fusion mRNA sequences with discordant chimeric paired reads. Finally,
we define an integration as a fusionless integrations if it has no
chimeric mRNA paired-end reads that can be used to determine
orientation. Using these definitions, we characterized XXX, XXX, and
XXX integrations as simmple, complex, and fusionless, respectively.
...
complex), there is a two- to five-fold increase in expression local to
the integration point (Fig.~\ref{updown}b).  The increase in
expression can be evident up to 100kb around the integration point
(Fig.~\ref{updown_250kb}).  In the case of simple integrations, there
is a clear increase downstream of the inetgrated viral sequence.
Finally, integrations without any fusion mRNA sequences show slight
decrease in expression.  These observations show that viral
integration generally increases local transcriptional activity
possibly driven by the viral regulatory elements. The driving factor
is agnostic to whatever is downstream of the integration, and can
result in transcription of elements that are normally silent or not
expressed in normal samples.
...
chr3:126849186 and HPV:6071 (Fig.~\ref{tcga_c5_a0tn}a). A relatively
simple explanation for this complex rearrangement is that it
represents extrachromosomal DNA (ecDNA), where fragments of the genome
(human and virus) exist in circular forms outside of the
chromosomes. Recent results have shown that ecDNA is found in nearly
half of all cancers, and that amplified driver oncogenes were commonly
found in ecDNA~\cite{Turner2017}. Thus, it is not implausible that
human-viral ecDNA might also exist, especially because HPV can exist
in both integrated and episomal forms within cervical
cells~\cite{Williams2011,Shin2014}.

Human-viral ecDNA would not be bound to the chromatin and would be
readily accessible for transcription, allowing it to transcribe freely
and unregulated.  While the majority of integrations do not occur near
oncogenes, our own observations show that there is an increase
transcription of LINE and LTR elements near integration sites, and
thus human-viral ecDNA structures that contain LINE/LTR elements might
provide selective advantages and would selected for amplification.
This observation might explain why there is local copy number
amplification often found flanking the integrated
regions~\ref{Akagi2014}, i.e., the flanking regions represent
amplified ecDNA.  This is an exciting hypothesis that warrants further
investigation.
...
integration~\cite{Tang2013,Lawrence2015,Hu2015,Zhang2016}.  Previous
...
can be independent amplification as well as allow for unregulated
...
The impact of viral integration on proximal transcription expression
is clear.  Our study highlights the need for more research on the role
of HPV integration on oncogenesis, and in particular, with more focus
on samples with non-hotspot integrations.  As the majority of
integrations are not occurring in genomic hotspots, mechanisms such as
increased LINE and LTR expressions and the formation of human-viral
ecDNA for amplification of oncogenic elements might play a crucial
role in tumor progression and genomic instability.
