...
Previous research has observed that genomic regions with HPV integrations were enriched for genes~\cite{Akagi2014} or long terminal repeat/endogenous retroviruses (LTR/ERV1)~\cite{Hu2015} elements.  We also observed an enrichment of genes near integration points (Z-test; p-value $<$ 3e-04), but also a depletion of all LTR elements near integration points (Z-test; p-value $<$ 3e-04).  In addition, we observed an enrichment of LINE near integration points (Z-test; p-value $<$ 6e-03).  Note that while our observations do deviate from the distribution of expected counts, the observed values still fall within the tail end of the distribution.  Finally, while many studies have noted that integrations do occur at or near oncogenes, these are very rare, and thus, can only explain tumorigenesis in the few cases in which they occur.   
...
{\label{integration_counts}  {\bf Observed and expected number of annotations of the integrated regions.}  We show the violin plots of the expected number of counts from random integrations for each annotation over 1000 simulated experiments, and the actual number of annotations for the observed integrations found in our TCGA-CESC dataset (in red).  The quantiles of the violin plots are displayed with lines.  The p-values of the observed annotation counts (Z-test) are: LINE $<$ 6e-03;	LTR $<$ 3e-04;  Gene $<$ 3e-04; and	Oncogenes $<$ 0.098.}
\end{figure}

We observed recurrent integrations (within 50kb of each other) in two loci: two samples with integrations between region chr13:73955151-74005092 and four samples with integrations between region chr8:128747810-128889296.  The first region contains no nearby genes.  The latter region contains the MYC and PVT1 genes and has previously been found to contain recurrent integrations from previous studies~\cite{Hu2015}.  However, the vast majority of integrations are non-recurrent and occur uniquely within that sample, and thus, focusing on the genomic hotspots found using recurrent integrations will exclude the majority of integrations and their roles on tumorigensis.

We found that 52\% of the genomic integrations (119 genomic integrations) had fusion mRNA sequences (392 fusion mRNA sequences).  While a few of the fusion mRNA breakpoints were found within close proximity of the genomic integration breakpoint (27\% were within 100 bps), the majority were within 1000 bps away (66\%; Fig.~\ref{distance_breakpoint}).  Thus, ViFi's results on two independent sequencing methods (WGS and RNA-seq) are in concordance and are evidence of its ability to recover both fusion and genomic integration events.
...
The previous result exploring the orientation of the fusion mRNA sequences showed a strong bias of human portions almost always being downstream of the viral genes.  This observation suggested that viral integration might result strongly impact expression near the integration point.  In order to confirm this hypothesis, for each integration we examined the transcription activity of the region within 10kb of the sample containing the integration, and then examined the mean transcriptional activity of the same region within the samples without that integration.  We then computed the log-fold change in expression as an estimate of the impact of integration on expression (Fig.~\ref{all_expression}).  The results show that integration typically results in either a neutral or increase in log-fold change in expression.  This change is more prominent when the integration also results in a fusion transcript, and can be as large as a 262 fold increase in expression compared to the same region in samples without integrations.  Such a large change can have a profound impact on regulatory networks.
