...
As we were unable to identify genomic hotspots as causal factors for tumorigenesis, we focused on the impact of integration on local transcription expression.  For each integration, we examined the transcription activity of the region within 10kb of the sample containing the integration and computed the log-fold change in the region's expression against the mean transcriptional activity of the same region for samples without that integration (Fig.~\ref{all_expression}).  We find an average 2.83 fold increase in expression of nearby human mRNA across all genomic segments with integrations, and moreover this change in expression is impacted on whether the integration resulted in a fusion transcript (Fig.~\ref{expression_transcripts}).  There is a 7.41 fold increase if the genomic segment contains a fusion mRNA sequence, and a 0.97 decrease when there is no fusion mRNA present.  Previous genome-wide surveys of viral integration sites explored the role of integration on gene expression~\cite{Sung2012,Hu2015}; our results show that integration typically results in upregulation of local transcriptional activity, regardless if genes are present or not, and this upregulation could potentially disrupt regulatory networks in unknown and unexpected ways.
...
The observation that integration often results in increased expression leaves us with two fundamental questions.  First, how does the increased expression contribute to pathogenesis.  Second, what causes the increase in expression, and in particular, why is this increase more prominent when fusion mRNA sequences are observed.  

To answer the first question, we began by annotating the transcripts that were expressed within the genomic segments.  For each genomic segment containing an integration, we took all positions within 10kb of the integration that were covered by at least three or more reads, and then clustered the positions into intervals.  We merged any intervals that were within 5 bps of another interval.  We then counted the number ofannotations that intersected any of the intervals using RefSeq genes~\cite{OLeary2016} or RepeatMasker~\cite{Tarailo-Graovac2009}) annotations.  We also computed the expected number of annotations observed from randomly selected segments as a baseline for comparison.  We found that regions with integrations were statistically significantly enriched for all annotations (Z-test; p-value < 1e-20), including a two-fold increase in the number of gene annotations of transcripts, and more interestingly, more than a 12-fold increase in LINE and LTR annotations of transcripts (Fig.~\ref{transcript_count}).  This is particularly surprising considering that we saw no statistically significant difference between the number of LINE and LTR annotation at the genomic level, however, we do see a statistically significant difference at the transcriptomic level.  

This result suggests one potential avenue for increased pathogenicity: genomic instability and pathway disruption via expression of LINE and LTR elements that are not normally expressed.  L1 elements are retrotransposons that have a ``cut and paste'' mechanism which allow them to replicate through RNA intermediates and re-insert back into genomic DNA.  L1 elements have been shown to create double stranded breaks in the host cell genome, which might lead to genomic instability~\cite{Gasior2006}.  L1 elements are typically epigenetically repressed in somatic cells~\cite{Kinomoto2007,Sigurdsson2012}, but they have been shown to be active in selected neoplasms~\cite{Rodic2013,Xiao-Jie2016}.  LTRs can act as promoters, enhancers, and transcriptional factor-binding sites, and can potentially be transcriptionally active, depending on the tissue~\cite{Yu2013}.  In most cases, LTRs are transcriptionally silenced by epigenetic mechanisms, however, in cancer, LTRs have been shown to have up-regulated expression (see~\cite{Romanish2010} for a review).  Our results show a four to five-fold increase in expression of these elements in regions proximal to integration sites (Fig.~\ref{expression_transcripts}).  As HPV randomly integrates into the genome, it might fall proximal to LINE and LTR elements and result in deregulation of cellular pathways by the active transcription of LINE and LTR elements, and in particular increased genomic instability via production of L1 elements.  
...
Key unanswered questions are why some integrations result in fusion mRNA transcripts and others do not, and why this difference causes a large discrepancy between expression levels for integrations with and without fusion transcripts.  Closer inspection of several integrations with fusion transcripts gave clues to these questions.  We found that for some integrations, that the segments from the human and viral genomes could be arranged in a cyclic structure (Fig.~\ref{tcga_c5_a0tn}).  For example, the sample TCGA-C5-A0TN had 235 chimeric paired-end reads between chr2:195586245 and HPV16:2593, 149 discordant paired-end reads between chr2:195603512 and chr3:126826267, and 229 chimeric paired-end reads between chr3:126849186 and HPV:6071 (Fig.~\ref{tcga_c5_a0tn}a).  These structures may represent extrachromosomal DNA (ecDNA).  Human-viral ecDNA would not be bound to the chromatin and would be readily accessible for transcription, allowing it to transcribe freely and unregulated.   We have recently shown that ecDNA is found in nearly half of all cancers, and that amplified driver oncogenes were commonly found in ecDNA~\cite{Turner2017}.  Thus, it would not be implausible human-viral ecDNA might also exist, especially because HPV can exist in both integrated and episomal forms within cervical cells~\cite{Williams2011,Shin2014}.  While the majority of integrations do not occur near oncogenes, our own observations show that there is an increase transcription of LINE and LTR elements near integration sites, and thus human-viral ecDNA structures that contain LINE/LTR elements might provide selective advantages and would selected for amplification.  This observation might explain why there is local copy number amplification often found flanking the integrated regions~\ref{Akagi2014}, i.e., the flanking regions represent amplified ecDNA.  This is an exciting hypothesis that warrants further investigation.  %Future fluorescence in situ hybridization (FISH) may help determine the structure of these integrations.
