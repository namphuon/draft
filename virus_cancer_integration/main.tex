...
points~(Z-test; p-value $<$ 1e-6; Fig.~\ref{integration_counts}).
...
and we only found significant enrichment of SINE elements~(Z-test; p-value $<$ 1e-7; Fig.~\ref{integration_counts}) proximal to integration sites.  Finally, while we did
...
expression ($2.83\times$ average increase; Wilcoxon signed rank test; p-value $<$ 1e-14) across all genomic
...
fusion transcript (Fig.~\ref{expression_transcripts}).  There is a statistically significant
increase ($7.41\times$ average increase; Wilcoxon signed rank test; p-value $<$ 1e-15) if the genomic segment contains a fusion mRNA sequence, versus a small but not significant decrease ($0.97\times$; Wilcoxon signed rank test; p-value $<$ 0.721) when no fusion mRNA is seen.
...
that genes, LINEs, SINEs, LTRs were all statistically significantly more likely to be transcribed in the
proximity of an integration site (Z-test; p-value $<$ 1e-20). Genes were twice as likely, and
...
\paragraph{\textbf{Annotation of genomic and transcriptomic regions.}}
